...
function loadConfig() {
  const missing = []
  const apiKey = configService.get('ramp.moonpay.apiKey') ?? ''
  if (!apiKey) missing.push('ramp.moonpay.apiKey')
  const signUrl = configService.get('ramp.moonpay.signUrl') ?? ''
  if (!signUrl) missing.push('ramp.moonpay.signUrl')
  const env = configService.get('ramp.moonpay.environment') ?? ''
