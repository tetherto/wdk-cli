...
export async function startMcpServer() {
  ...
  async function handleRamp(direction, { network, token, fiatCurrency, fiatAmount, cryptoAmount, index, wallet }) {
    try {
      const result = await createRampUrl({ direction, network, index, token, fiatCurrency, fiatAmount, cryptoAmount, wallet })
      return jsonResult(result)
    } catch (e) {
      return errorResult(e)
    }
